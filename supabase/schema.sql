...
create index if not exists prayer_contents_user_id_prayer_id_idx
  on public.prayer_contents (user_id, prayer_id, registered_at desc, id desc);

create unique index if not exists prayer_contents_user_id_client_id_idx
  on public.prayer_contents (user_id, client_id)
  where client_id is not null;
